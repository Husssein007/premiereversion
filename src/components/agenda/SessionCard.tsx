...
const SessionCard = ({
  ...
  return <div className="flex items-start gap-6 relative group">
      <div className="timeline-line" />
      <TimeBadge time={time} />
      <div className="flex-1 session-card transition-all duration-300 hover:shadow-md hover:border-primary/40 hover:translate-x-1">
        ...
        {description && <p className="text-muted-foreground mb-6 leading-relaxed text-[15px]">
            {description}
          </p>}

        {speakers && speakers.length > 0 || moderator || keynote ? <div className="mt-8 space-y-8">
            {/* Keynote speaker - même style que les speakers */}
            {keynote && <div className="flex items-start gap-4 py-3">
            ...
            {/* Panélistes */}
            {speakers && speakers.length > 0 && <div>
                {!hideSpeakersLabel && <h4 className="text-base font-bold text-[hsl(331,52%,53%)] mb-5 small-caps tracking-wide underline">
                    Panélistes
                  </h4>}
              ...
              return <div key={index} className={isLastOdd ? "md:col-span-2 md:flex md:justify-center" : ""}>
            ...
            {moderator && <div>
                ...
                <div className="rounded-xl shadow bg-orange-200 gap-0 items-center justify-start flex flex-col py-[16px] border border-orange-600">
                  <div className="flex-shrink-0 w-12 h-12 border border-[hsl(25,80%,70%)] rounded-md bg-[hsl(25,80%,97%)] flex items-center justify-center px-0 mx-[168px] gap-0">
                    <User className="w-6 h-6 text-[hsl(25,80%,50%)]" />
                  </div>
                  <div className="flex-1 min-w-0 pt-0.5 py-0 my-0 mx-0">
                    <p className="font-bold text-[15px] leading-snug mb-1.5 tracking-wide text-orange-600 mx-0 my-0 text-center">
                      {moderator.name}
                    </p>
                    {moderator.role && <p className="text-[13px] leading-relaxed text-[#ed6e0c]">{moderator.role}</p>}
                  </div>
                </div>
              </div>}
          </div> : null}
